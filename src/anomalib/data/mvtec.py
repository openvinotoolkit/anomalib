...
class MVTecDatasetEnsemble(AnomalibDataset):
    """MVTec dataset class.
    ...
    def __getitem__(self, index: int) -> dict[str, str | Tensor]:
        ...
        print("tiling")
        tiled_images = self.tiler.tile(item["image"])
        # add channel so it fits tiler input
        tiled_masks = self.tiler.tile(item["mask"].unsqueeze(0))

        # remove batch
        item["image"] = tiled_images[self.tile_index].squeeze(0)
        # remove batch and channel
        item["mask"] = tiled_masks[self.tile_index].squeeze(0).squeeze(0)

        return item


class MVTecEnsemble(AnomalibDataModule):
    """MVTec Datamodule used with ensemble of models
    ...
    def __init__(
        ...
        self.train_data = MVTecDatasetEnsemble(
            task=task, transform=transform_train, split=Split.TRAIN, root=root, category=category,
            tiler=tiler, tile_index=tile_index
        )
        self.test_data = MVTecDatasetEnsemble(
            task=task, transform=transform_eval, split=Split.TEST, root=root, category=category,
            tiler=tiler, tile_index=tile_index
        )