...
This module provides PyTorch Dataset implementation for the MVTec LOCO dataset. The
...
class MVTecLOCODataset(AnomalibDataset):
    ...
    Dataset class for loading and processing MVTec LOCO dataset images. Supports
...
def make_mvtec_loco_dataset(
    ...
    """Create MVTec LOCO samples by parsing the data directory structure.
    ...
    Returns:
        DataFrame: Dataset samples with columns:
            ...
            - anomaly_type: Type of anomaly (structural/logical) for abnormal samples

    Example:
        >>> root = Path("./datasets/MVTec_LOCO/breakfast_box")
        >>> samples = make_mvtec_loco_dataset(root, split="train")
        >>> samples.head()
           path                split label image_path           mask_path label_index anomaly_type
        0  datasets/MVTec_LOCO/breakfast_box train good  [...]/good/105.png           0
        1  datasets/MVTec_LOCO/breakfast_box train good  [...]/good/017.png           0
    ...
    samples["anomaly_type"] = ""
    for anomaly_type in ANOMALY_TYPES:
        samples.loc[samples.label == anomaly_type, "anomaly_type"] = anomaly_type
