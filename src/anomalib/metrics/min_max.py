...
class _Min(Metric):
    """Track minimum value across batches.

    This metric maintains running minimum values across all batches
    it processes. It is useful for tasks like normalization or monitoring the
    range of values during training.
...
class _Max(Metric):
    """Track maximum value across batches.

    This metric maintains running maximum values across all batches
    it processes. It is useful for tasks like normalization or monitoring the
    range of values during training.
    ...
    def update(self, predictions: torch.Tensor, *args, **kwargs) -> None:
        ...
        self.max = torch.max(self.max, torch.min(predictions))
...
class Min(AnomalibMetric, _Min):  # type: ignore[misc]
    """Wrapper to add AnomalibMetric functionality to Min metric."""


class Max(AnomalibMetric, _Max):  # type: ignore[misc]
    """Wrapper to add AnomalibMetric functionality to Max metric."""
