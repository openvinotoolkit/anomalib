...
class AnomalyMapGenerator(nn.Module):
    ...
    def compute_anomaly_mask(
        ...
        target_size: Union[int, None] = None,
        upsample_mode="bilinear",
    ):
        """This method is not used in the basic functionality of training and testing.

        It is included as it is part of the U-Flow paper, and can be called separately if an unsupervised anomaly
        ...
        hypothesis, which is that all pixels are normal. First, we create a level set tree, and compute the Probability of
        False Alarms (PFA) for all nodes. Then we apply a pruning and merging process to the tree, to remove nodes with
        high PFA values. The final tree is used to compute the log-NFA, which is finally used to obtain the segmentation
        mask with an automatic threshold.
        ...
        Args:
            ...
            log_nfa_threshold (float): Log-NFA threshold for the anomaly mask. Defaults to 0.
            target_size (int): Target size for the output anomaly mask. Defaults to self.input_size.
            upsample_mode (str): Upsample mode for merging results at different scales. Defaults to 'bilinear'.

        Returns:
            detection mask. Tensor of shape (N, 1, target_size, target_size), where N is the batch size.
            Anomaly score based on the log-NFA value (-log NFA). Same shape as the detection mask.
        """
