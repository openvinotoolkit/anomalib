...
@app.route("/")
def index():
    """Main URL of App and require login.
...
def login():
    """User login and redirect homepage
    """
...
def signup():  # define the sign up function
    """User sign up and redirect homepage
    """
...
def inferencing():
    ...
    if request.method == "POST" and current_user:
        if not request.files["image_input"] or not request.form["user_category"]:
            ...
            inferencer = TorchInferencer(config="configs/" \
                    + user_category + ".yaml", model_source="models/" \
                    + user_category + ".ckpt")
            ...
            db.session.commit()
            return render_template("result_inference.html", \
                name=current_user.name, image_upload=image_upload_path, \
                anomaly_score=predictions.pred_score, target=predictions.pred_label, \
                heatmap_predict=heatmap_predict, mask_predict=mask_predict, \
                segment_predict=segment_predict)
...
def database():
    """Database of save account user and results of inference
    Return:
...
def delete(id: int):
    """
    Function to delete a row in database result
    Return:
