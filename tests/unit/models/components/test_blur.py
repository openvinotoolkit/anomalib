...
def test_blur_equivalence(kernel_size, sigma, channels, device):
    for _ in range(10):
        input_tensor = torch.randn((3, channels, 128, 128)).to(device)
        kornia = korniaGaussianBlur2d(kernel_size, sigma, separable=False).to(device)
        blur_kornia = kornia(input_tensor)
        gaussian = GaussianBlur2d(kernel_size=kernel_size, sigma=sigma, channels=channels).to(device)
