...
def sweep(
    ...
    output = f"One sweep run complete for model {run_config.model_name}"
    output += (
        f" On category {model_config.data.init_args.category}"
        if model_config.data.init_args.category is not None
        else ""
    )
...
def hide_output(func):
    ...
    def wrapper(*args, **kwargs):
        with redirect_stdout(io.StringIO()) as out_buf, redirect_stderr(io.StringIO()) as err_buf:
            ...
            try:
                ...
                raise Exception(out_buf.getvalue() + err_buf.getvalue()) from exp
...
def distribute(config_path: Path):
    ...
    Args:
        config_path: (Path): Sweep configuration.
    """

    # suppress_logging(suppress_logs)
    ...
    if not torch.cuda.is_available() and "gpu" in devices:
        logging.warning("Config requested GPU benchmarking but torch could not detect any cuda enabled devices")
