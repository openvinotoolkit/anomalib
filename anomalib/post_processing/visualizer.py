...
class Visualizer:
    ...
    def __init__(self, mode: str, task: str, top_k_images: int = 5):
    ...
    def visualize_batch(self, batch: Dict, anomalib_module: AnomalyModule) -> Iterator[np.ndarray]:
        ...
        if "selected_features" in batch.keys():
            selected_featuremaps = batch["selected_features"]
        for i in range(batch["image"].size(0)):
            ...
            yield self.visualize_image(image_result, anomalib_module, selected_featuremaps, i)

    def visualize_image(
        self,
        image_result: ImageResult,
        anomalib_module: AnomalyModule,
        selected_featuremaps: Dict[str, Tensor],
        image_index: int,
    ) -> np.ndarray:
        ...
        """
        if self.mode == "full":
            return self._visualize_full(image_result, anomalib_module, selected_featuremaps, image_index)
        if self.mode == "simple":
    ...
    def _visualize_full(
        ...
        if self.task == "segmentation":
            ...
            threshold = anomalib_module.pixel_metrics.threshold
            if selected_featuremaps is not None:
                for label in selected_featuremaps.keys():
                    subclass_features = selected_featuremaps[label][image_index]
                    all_features = (
                        anomalib_module.model.anomaly_map_generator.feature_to_anomaly_map(
                            subclass_features, feature=-1
                        )
                        .cpu()
                        .numpy()
                    )
                    all_features_heatmap = superimpose_anomaly_map(all_features, image_result.image, normalize=True)
                    visualization.add_image(image=all_features_heatmap, title=f"All {label}")
                    ...
                    all_features_vis_image = mark_boundaries(
                        image_result.image, all_features_pred_mask, color=(1, 0, 0), mode="thick"
                    )
                    visualization.add_image(image=all_features_vis_image, title=f"All {label}")
                    # for idx in range(self.top_k_images):
                    #     selected_feature = (
                    #         anomalib_module.model.anomaly_map_generator.feature_to_anomaly_map(
                    #             subclass_features, feature=idx
                    #         )
                    #         .cpu()
                    #         .numpy()
                    #     )
