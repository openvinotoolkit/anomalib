...
def get_feature_extractor(backbone: str):
    if backbone == "wide_resnet50_2":
        ...
        feature_extractor = EfficientNet.from_pretrained("efficientnet-b5")
    elif backbone == "vgg19_bn":
        feature_extractor = vgg19_bn(pretrained=True, progress=True)
    else:
        raise ValueError(f"Supported backbones are {SUPPORTED_BACKBONES}. Got {backbone} instead.")

    return feature_extractor


class CfaModel(nn.Module):
    def __init__(self, backbone, data_loader, gamma_c, gamma_d, device):
