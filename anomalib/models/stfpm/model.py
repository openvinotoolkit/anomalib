...
class STFPMModel(pl.LightningModule):
    ...
    def test_epoch_end(self, outputs):
        ...
        for (filename, image, true_mask, anomaly_map) in zip(
            ...
            # Not a good idea to put it here. Will upload too many images
            self.logger.log_image(name=f"{filename}_Image", image=image) # bgr to rgb
            self.logger.log_image(name=f"{filename}_Predicted Heat Map", image=heat_map)
            self.logger.log_image(name=f"{filename}_Segmentation Result", image=vis_img)
            self.logger.log_image(name=f"{filename}_Predicted Mask", image=pred_mask)
